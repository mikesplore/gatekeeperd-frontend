...
export function DataTable<T>({ data, columns, filters = [], getRowKey, emptyMessage = "No records found.", pageSize = 10, searchPlaceholder = "Search…", isLoading = false, loadingRows = 5 }: { data: T[]; columns: DataTableColumn<T>[]; filters?: DataTableFilter<T>[]; getRowKey: (row: T) => string; emptyMessage?: string; pageSize?: number; searchPlaceholder?: string; isLoading?: boolean; loadingRows?: number }) {
  ...
  return <div className="space-y-3">
    ...
    <div className="flex items-center justify-between border-t pt-4 pb-1 text-xs text-muted-foreground"><span>{isLoading ? "Loading records…" : `${filtered.length} result${filtered.length === 1 ? "" : "s"}`}</span><div className="flex gap-1.5"><Button variant="outline" size="sm" className="h-8 px-2 text-xs" disabled={isLoading || currentPage === 0} onClick={() => setPage((value) => value - 1)}>Previous</Button><span className="px-1.5 py-1.5">{isLoading ? "..." : `${currentPage + 1} / ${pageCount}`}</span><Button variant="outline" size="sm" className="h-8 px-2 text-xs" disabled={isLoading || currentPage >= pageCount - 1} onClick={() => setPage((value) => value + 1)}>Next</Button></div></div>
  </div>;
}
