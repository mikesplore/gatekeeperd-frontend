...
export function CustomersPage() {
  ...
  return <QueryState isLoading={query.isLoading || sites.isLoading} isError={query.isError || sites.isError} error={query.error ?? sites.error} data={query.data}>{customers => <div className="space-y-6"><div><h1 className="text-2xl font-semibold">Customers</h1><p className="text-muted-foreground">Ownership and aggregate site health.</p></div><div className="grid gap-6 lg:grid-cols-2"><Card><CardHeader><CardTitle>New customer</CardTitle></CardHeader><CardContent><form className="space-y-3" onSubmit={async event => { event.preventDefault(); const form = new FormData(event.currentTarget); try { await create.mutateAsync({ name: String(form.get("name")), contactEmail: String(form.get("email") || "") || undefined, contactPhone: String(form.get("phone") || "") || undefined }); toast.success("Customer created"); event.currentTarget.reset(); } catch (error) { toast.error(getApiErrorMessage(error)); } }}><Input name="name" placeholder="Customer name" required /><Input name="email" type="email" placeholder="Contact email" /><Input name="phone" placeholder="Contact phone" /><Button disabled={create.isPending}>{create.isPending ? "Creating…" : "Create customer"}</Button></form></CardContent></Card><Card><CardHeader><CardTitle>Assign project</CardTitle></CardHeader><CardContent><form className="space-y-3" onSubmit={async event => { event.preventDefault(); const form = new FormData(event.currentTarget); try { await assign.mutateAsync({ projectId: String(form.get("projectId")), customerId: String(form.get("customerId")) }); toast.success("Project assigned"); } catch (error) { toast.error(getApiErrorMessage(error)); } }}><select name="projectId" required className="h-9 w-full rounded-md border bg-transparent px-3 text-sm"><option value="">Select project</option>{(projects.data ?? []).map(project => <option key={project.id} value={project.id}>{project.name} ({project.slug})</option>)}</select><select name="customerId" required className="h-9 w-full rounded-md border bg-transparent px-3 text-sm"><option value="">Select customer</option>{customers.map(customer => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select><Button disabled={assign.isPending}>{assign.isPending ? "Assigning…" : "Assign project"}</Button></form></CardContent></Card></div><div className="grid gap-4">{customers.map(customer => { const open = expanded === customer.id; return <Card key={customer.id}><CardHeader className="cursor-pointer" onClick={() => setExpanded(open ? null : customer.id)}><div className="flex items-start justify-between gap-3"><div className="flex items-start gap-2">{open ? <ChevronDown className="mt-0.5 h-4 w-4" /> : <ChevronRight className="mt-0.5 h-4 w-4" />}<div><CardTitle>{customer.name}</CardTitle><p className="mt-1 text-sm text-muted-foreground">{customer.contactEmail ?? "No email"} · {customer.siteCount} sites</p></div></div><StatusSummary health={customer.health} /></div></CardHeader>{open && <CardContent><CustomerSites id={customer.id} sites={sites.data} /><Link className="mt-3 inline-block text-sm text-primary hover:underline" to={`/app/customers/${customer.id}`}>Open customer detail →</Link></CardContent>}</Card>; })}</div></div>}</QueryState>;
}
