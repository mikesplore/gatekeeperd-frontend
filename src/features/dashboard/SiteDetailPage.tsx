...
export function SiteDetailPage() {
  ...
  return <QueryState isLoading={query.isLoading} isError={query.isError} error={query.error} data={query.data}>
    {detail => <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2"><Button variant="outline" asChild><a href={`https://${detail.site.domain}`} target="_blank" rel="noreferrer"><ExternalLink className="mr-2 h-4 w-4" />Visit site</a></Button><Button variant="outline" onClick={() => setTab("settings")}><Settings2 className="mr-2 h-4 w-4" />Settings</Button><Button variant="destructive" size="icon" aria-label="Delete site" onClick={() => setDeleteOpen(true)}><Trash2 className="h-4 w-4" /></Button></div>
      </div>
