...
import { Check, Clock3, ExternalLink, FileCode2, LockKeyhole, MoreHorizontal, Server, ShieldAlert, Trash2 } from "lucide-react";
  ...
  return <QueryState isLoading={query.isLoading} isError={query.isError} error={query.error} data={query.data}>
    {detail => <div className="w-full space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <p className="text-sm text-muted-foreground">Manage this site’s health, routing, and configuration.</p>
        <div className="ml-auto flex flex-wrap items-center justify-end gap-2"><Button variant="outline" asChild><a href={`https://${detail.site.domain}`} target="_blank" rel="noreferrer"><ExternalLink className="mr-2 h-4 w-4" />Visit site</a></Button><Button variant="outline" asChild><Link to={`/app/nginx/sites/${encodeURIComponent(detail.site.slug)}/advanced`}><FileCode2 className="mr-2 h-4 w-4" />Advanced config</Link></Button><Button variant="destructive" size="icon" aria-label="Delete site" onClick={() => setDeleteOpen(true)}><Trash2 className="h-4 w-4" /></Button></div>
      </div>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="h-auto w-full justify-start gap-1 overflow-x-auto rounded-none border-b bg-transparent p-0 text-muted-foreground"><TabsTrigger value="overview" className="rounded-none border-b-2 border-transparent px-4 py-3 data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none">Overview</TabsTrigger><TabsTrigger value="history" className="rounded-none border-b-2 border-transparent px-4 py-3 data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none">History</TabsTrigger><TabsTrigger value="settings" className="rounded-none border-b-2 border-transparent px-4 py-3 data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none">Settings</TabsTrigger></TabsList>

        <TabsContent value="overview" className="space-y-4 pt-2">
          <div className="grid gap-4 lg:grid-cols-3">
            <Card className="lg:col-span-2"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle>Site health</CardTitle><CardDescription>Current proxy and upstream state.</CardDescription></div><Badge variant="outline" className={`capitalize ${statusClasses[detail.site.status]}`}>{detail.site.status.replace(/_/g, " ")}</Badge></div></CardHeader><CardContent className="grid gap-5 sm:grid-cols-2"><div><p className="text-xs uppercase tracking-wide text-muted-foreground">Upstream health</p><p className="mt-1 font-medium capitalize">{detail.site.dockerState ?? "Not reported"}</p></div><div><p className="text-xs uppercase tracking-wide text-muted-foreground">Configuration file</p><p className="mt-1 font-medium">{detail.site.available ? "Available" : "Missing"}</p></div><div><p className="text-xs uppercase tracking-wide text-muted-foreground">Enabled link</p><p className="mt-1 font-medium">{detail.site.enabled ? "Active" : "Inactive"}</p></div><div><p className="text-xs uppercase tracking-wide text-muted-foreground">TLS certificate</p><p className="mt-1 font-medium">{detail.certificateInstalled ? `Installed${detail.certificateDaysRemaining != null ? ` · ${detail.certificateDaysRemaining} days remaining` : ""}` : detail.certificateInstalled == null ? "Unknown" : "Not installed"}</p></div></CardContent></Card>
