...
export function NotificationsPage() {
  ...
  const notifications: any = useNotifications(limit, offset);
  const action = useNotificationAction(); const severity = params.get("severity") ?? "all"; const search = params.get("q") ?? "";
  const update = (key: string, value: string) => { const next = new URLSearchParams(params); value && value !== "all" ? next.set(key, value) : next.delete(key); if (key !== "offset") next.delete("offset"); setParams(next); };
  useEffect(() => {
