  ...
  return <div className="space-y-6">
    <QueryState isLoading={query.isLoading} isError={query.isError} error={query.error} data={query.data}>
      {(container) => <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card className="lg:col-span-2"><CardHeader><CardTitle>Volumes and mounts</CardTitle></CardHeader><CardContent>{container.volumes?.length ? <div className="space-y-2">{container.volumes.map((mount) => <div key={`${mount.hostPath}:${mount.containerPath}`} className="grid gap-1 rounded-md border p-3 text-sm sm:grid-cols-[1fr_auto_1fr_auto] sm:items-center"><MountSource value={mount.hostPath} /><span className="text-muted-foreground">to</span><span className="font-mono break-all">{mount.containerPath}</span><Badge variant="outline">{mount.readOnly ? "Read only" : "Read/write"}</Badge></div>)}</div> : <p className="text-sm text-muted-foreground">No volumes or bind mounts attached.</p>}</CardContent></Card>
...
function MountSource({ value }: { value: string }) {
  const isNamedVolume = !value.startsWith("/") && !value.startsWith(".");
  return isNamedVolume ? <Link className="break-all font-mono text-primary hover:underline" to={`/app/volumes/${encodeURIComponent(value)}`}>{value}</Link> : <span className="break-all font-mono">{value}</span>;
}
