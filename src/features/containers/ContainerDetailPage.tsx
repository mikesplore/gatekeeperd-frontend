  ...
  return <div className="space-y-6">
    ...
    <QueryState isLoading={query.isLoading} isError={query.isError} error={query.error} data={query.data}>
      {(container) => <div className="grid gap-4 lg:grid-cols-2">
        <Card><CardHeader><CardTitle>Runtime</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2"><Info label="Image" value={container.image} mono /><Info label="Status" value={container.status} /><Info label="State" value={container.state} badge /><Info label="Restart policy" value={container.restartPolicy ?? "Unknown"} /><Info label="Ports" value={container.ports || "Not set"} mono /></CardContent></Card>
        <Card><CardHeader><CardTitle>Networks</CardTitle></CardHeader><CardContent>{container.networks?.length ? <div className="flex flex-wrap gap-2">{container.networks.map(network => <Badge key={network} variant="outline">{network}</Badge>)}</div> : <p className="text-sm text-muted-foreground">No networks attached.</p>}</CardContent></Card>
