...
export function DockerResourcesPage({ kind }: { kind: "networks" | "volumes" }) {
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle>Available {kind}</CardTitle></CardHeader><CardContent><QueryState isLoading={query.isLoading} isError={query.isError} error={query.error} data={query.data}>{items => kind === "networks" ? <DataTable data={items as DockerNetwork[]} getRowKey={item => item.name} searchPlaceholder="Search networks..." columns={[{ key: "name", header: "Name", searchable: true, searchValue: item => item.name, render: item => <Link className="font-medium text-primary hover:underline" to={`/app/networks/${encodeURIComponent(item.name)}`}>{item.name}</Link> }, { key: "driver", header: "Driver", render: item => <Badge variant="secondary">{item.driver}</Badge> }, { key: "scope", header: "Scope", render: item => item.scope }, { key: "ipam", header: "IPAM", render: item => <span className="text-xs">{item.gateway ?? "No gateway"}{item.subnet ? ` · ${item.subnet}` : ""}</span> }, { key: "containers", header: "Containers", render: item => item.containers.length ? <div className="flex flex-wrap gap-1">{item.containers.slice(0, 3).map(container => <Badge key={container} variant="outline" className="font-normal">{container}</Badge>)}{item.containers.length > 3 && <Badge variant="secondary">+{item.containers.length - 3}</Badge>}</div> : <span className="text-muted-foreground">0</span> }, { key: "actions", header: "", render: item => !builtIn(item.name) ? <Button size="icon" variant="ghost" className="text-muted-foreground hover:text-destructive" title={`Delete ${item.name}`} onClick={() => setDeleteTarget(item.name)}><Trash2 className="h-4 w-4" /></Button> : <span /> }]} /> : <DataTable data={items as DockerVolume[]} getRowKey={item => item.name} searchPlaceholder="Search volumes..." columns={[{ key: "name", header: "Name", searchable: true, searchValue: item => item.name, render: item => <Link className="font-medium text-primary hover:underline" to={`/app/volumes/${encodeURIComponent(item.name)}`}>{item.name}</Link> }, { key: "driver", header: "Driver", render: item => item.driver }, { key: "scope", header: "Scope", render: item => item.scope }, { key: "mountpoint", header: "Mount point", render: item => <span className="font-mono text-xs">{item.mountpoint}</span> }, { key: "containers", header: "Mounted by", render: item => item.containers?.length ? item.containers.join(", ") : <span className="text-muted-foreground">0</span> }, { key: "actions", header: "", render: item => <Button size="icon" variant="ghost" className="text-destructive" title={`Delete ${item.name}`} onClick={() => setDeleteTarget(item.name)}><Trash2 className="h-4 w-4" /></Button> }]} />}</QueryState></CardContent></Card>
